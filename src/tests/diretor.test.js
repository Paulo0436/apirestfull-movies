...
const url = '/api/v1/movie/diretor';
...
let nome = `DiretorTest${tempo}`;
let idade = numeroAleatorio;
let nacionalidade = `Nacionalidade${tempo}`;

describe('Testes das rotas de DIRETOR', () => {
    ...
    test('POST:422 nome inexistente', async () => {
        ...
        expect(resposta.body.msg).toContain("Nome do diretor é obrigatório");
    });

    test('POST:422 idade inválida', async () => {
        const resposta = await request.post(url)
            .set("authorization", `Bearer ${token}`)
            .send({
                nome: nome,
    ...
    test('POST:422 idade inexistente', async () => {
        ...
        expect(resposta.status).toBe(422);
        expect(resposta.headers['content-type']).toMatch(/json/);
        expect(resposta.body.msg).toContain("Idade do diretor é obrigatória");
    });
    ...
    test('GET:id:404 não encontrado', async () => {
    ...
    test('PUT:id:404 não encontrado', async () => {
    ...
    test('DELETE:id:404 não encontrado', async () => {
