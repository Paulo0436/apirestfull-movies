...
const url = '/api/v1/movie/formato';
...
let nome = `nomeFormatoTest${tempo}`;

describe('Testes das rotas de FORMATO', () => {
    ...
    test('POST:422 nome inexistente', async () => {
        ...
        expect(resposta.body.msg).toContain("Nome do formato é obrigatório");
    });

    test('POST:401 token ausente', async () => {
        const resposta = await request.post(url).send({ nome });
    ...
    test('GET:id:404 id não encontrado', async () => {
        ...
        expect(resposta.headers['content-type']).toMatch(/json/);
        expect(resposta.body.msg).toContain("Formato não encontrado");
    });
